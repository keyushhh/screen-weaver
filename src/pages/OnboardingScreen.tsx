...
import biometricSection from "@/assets/biometric-section.png";
      ...
      <div className={`px-6 pb-8 space-y-6 ${showMpinSetup ? 'flex-1 flex flex-col pt-12' : ''}`}>
        ...
        {showMpinSetup && (
          <div className="space-y-6 animate-fade-in flex-1 flex flex-col">
                ...
                render={({ slots }) => (
                  <InputOTPGroup className="w-[364px] justify-between">
                    {slots.map((slot, index) => (
                      <div
                        key={index}
                        className={`relative flex items-center justify-center h-[54px] w-[81px] rounded-[12px] border-none text-2xl font-semibold text-white transition-all bg-cover bg-center ${
            ...
            {/* Biometric Toggle */}
            <div
              className="flex items-center justify-between px-4 w-full h-[54px] border-none bg-contain bg-center bg-no-repeat"
              style={{
                width: '364px', // Explicit width as requested
                backgroundImage: `url(${biometricSection})`
              }}
            >
              <div className="flex items-center gap-3">
                <img src={biometricIcon} alt="" className="w-6 h-6" />
